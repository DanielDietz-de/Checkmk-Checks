...
- Canonical package: `windows_ping` version `1.0.5`; minimum Checkmk version `2.3.0b1`; maximum asserted version: not asserted; validate on the target release.
- Canonical manifest: `windows_ping/src/info`; it declares 2 packaged files.
- Repository MKP artifacts present: `windows_ping-1.0.1.mkp`, `windows_ping-1.0.2.mkp`, `windows_ping-1.0.3.mkp`, `windows_ping-1.0.4.mkp`, `windows_ping-1.0.5.mkp`.
- No committed checksum file is present; do not distribute an unverified locally built artifact.
- Source under `src/` is authoritative; generated MKP files and this reference must match it.

### Configuration and components

- **Other packaged source:** `src/agents/custom/windows_ping/bin/check_ping.cmd`, `src/agents/custom/windows_ping/bin/check_ping.ps1`.
...
- Package-specific tests: `tests/test_windows_ping_integrity.py`.
