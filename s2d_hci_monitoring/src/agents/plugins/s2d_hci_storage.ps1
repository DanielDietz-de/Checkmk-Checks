...
.DESCRIPTION
    ...
    records to the logical cluster piggyback host. Storage-pool-derived sections
    are explicitly limited to clustered, non-primordial pools so node-local OS,
    recovery, and unrelated Storage Spaces objects cannot migrate between logical
    services when collector leadership changes. The collector is read-only.
#>
...
try {
    ...
    if ($clusterContext.IsLeader) {
        ...
        try {
            Get-StoragePool -ErrorAction Stop |
                Where-Object { -not $_.IsPrimordial -and [bool]$_.IsClustered } |
        ...
        try {
            Get-StoragePool -ErrorAction Stop |
                ...
                ForEach-Object {
                    $pool = $_
                    Get-VirtualDisk -StoragePool $pool -ErrorAction Stop |
                        Sort-Object FriendlyName |
                        ForEach-Object {
                            $source = if ($_.UniqueId) { [string]$_.UniqueId } elseif ($_.ObjectId) { [string]$_.ObjectId } else { [string]$_.FriendlyName }
                            [pscustomobject]@{
                                ...
                                friendly_name = [string]$_.FriendlyName
                                storage_pool = [string]$pool.FriendlyName
                                health_status = $_.HealthStatus.ToString()
                                operational_status = ($_.OperationalStatus -join ',')
        ...
        try {
            Get-StoragePool -ErrorAction Stop |
                ...
                ForEach-Object {
                    $pool = $_
                    Get-Volume -StoragePool $pool -ErrorAction Stop |
                        Sort-Object FileSystemLabel, DriveLetter |
                        ForEach-Object {
                            $drive = if ($_.DriveLetter) { "$($_.DriveLetter):" } else { $null }
                            $stableSource = if ($_.UniqueId) { [string]$_.UniqueId } elseif ($_.ObjectId) { [string]$_.ObjectId } else { [string]$_.Path }
                            if ([string]::IsNullOrWhiteSpace($stableSource)) {
                                throw "Clustered volume in storage pool '$($pool.FriendlyName)' has no stable identity source."
                            }
                            $label = [string]$_.FileSystemLabel
                            $record = [ordered]@{
                                identity = "vol-$(Get-S2DHciStableHash -Value $stableSource)"
        ...
        try {
            Get-StoragePool -ErrorAction Stop |
                ...
                ForEach-Object {
                    $pool = $_
                    Get-PhysicalDisk -StoragePool $pool -ErrorAction Stop |
                        Sort-Object FriendlyName, DeviceId |
                        ForEach-Object {
