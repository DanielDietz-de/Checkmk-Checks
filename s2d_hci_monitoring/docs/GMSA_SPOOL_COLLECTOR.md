...
When an update changes the configured spool filename—most commonly because `IntervalMinutes` changes—the installer reads the previous generated configuration **before replacing it**, confines the previous `spool_file` to the trusted spool directory, and removes the old snapshot before switching to the new path. It also removes a stale file already present at the new target path. This prevents Checkmk from ingesting two package snapshots with different TTL prefixes and potentially different `run_id` values.

The generated spool configuration path itself is an installation identity boundary. Before updating an existing root scheduled task, the installer reads the task action and recovers its registered `-ConfigPath`. That path must be uniquely recoverable, remain below the trusted Checkmk `config` directory, exist, and match the requested `-ConfigPath`. An in-place `ConfigPath` change is rejected; remove the task with generated state and reinstall it at the new path instead. This fail-closed rule prevents an old configuration and its spool snapshot from remaining active after the task starts using a different configuration file.
...
The previous spool remains untouched on a normal collector/runtime failure. Configuration lifecycle changes are different: when the operator intentionally changes the spool path, the installer retires the old generated snapshot so only one package snapshot remains active. The interval-derived numeric spool prefix provides Checkmk stale-data expiry independently of scheduled-task success without expiring before a valid schedule can refresh it.
...
`Remove-S2DHciVirtualizationCollectorTask.ps1` removes the scheduled task by default and leaves generated state untouched. With `-RemoveGeneratedState`, it first reads the generated spool configuration to recover and validate the currently configured custom/derived spool path, enumerates package-standard `^\d+_s2d_hci_virtualization\.txt$` snapshots in the trusted spool directory, removes those snapshots, and removes the configuration last. This also cleans legacy derived-lifetime files that may predate an interval change. The removal path honors `ShouldProcess`/`-WhatIf` and never deletes a configured spool path outside the trusted Checkmk spool directory.
