...
The update path is intentionally fail-closed. If an error occurs after an existing task has been quiesced but before the replacement task is registered, the old task remains disabled rather than resuming publication against partially changed state. Correct the reported error and rerun the installer. A successful registration explicitly enables the replacement root task.
