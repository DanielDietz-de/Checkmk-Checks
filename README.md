...
The repository-wide release workflow currently discovers **100 active packages** from their canonical `*/src/info` manifests. Pull requests validate only the unambiguously affected packages when that is safe; shared, ambiguous, manual, and `master` changes receive complete repository validation. Full validation runs all package tests, builds deterministic MKP archives, verifies checksums and package inventories, and validates supported packages in clean Checkmk 2.4 and 2.5 sites.
