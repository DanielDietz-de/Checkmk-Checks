...
Executable source, registered Checkmk objects, canonical manifests, tests, and workflow evidence drive documentation. Review those artifacts before editing prose. When prose conflicts with code, fix the implementation first if it is defective; otherwise correct the prose. Never change code merely to preserve an unsupported historical statement.

Every package README contains a marked operational reference generated by `tools/ci/generate_package_reference.py`. Edit source or `src/info`, update focused tests and human rationale, then regenerate the block. CI rejects hand-edited or stale generated references.
...
- Were actual paths, registrations, fields, defaults, service names, transport boundaries, and state behavior derived from current code rather than copied from older prose?
