...
_GUARD_IMPLEMENTATION = Path("tools/ci/repository_guard.py")
_SECURITY_REVIEW_MARKER = "# security-reviewed:"
...
def changed_files(root: Path, base: str | None, head: str | None) -> list[Path]:
    ...
    if base and head and set(base) != {"0"}:
        ...
        # behind, causing the guard to inspect packages the PR never touched.
        output = git_output(
            root,
            "diff",
            "--name-only",
...
def _is_guard_fixture(relative: Path) -> bool:
    """Return whether policy tokens are intentional test/guard source literals."""

    return "tests" in relative.parts or relative == _GUARD_IMPLEMENTATION


def _source_language(path: Path) -> str | None:
    """Classify normal source files and extensionless shebang executables."""
    ...
    if "python" in first_line:
        return "python"
    if any(shell in first_line for shell in ("/sh", "bash", "dash", "ksh", "zsh")):
        return "shell"
    if "ruby" in first_line or "php" in first_line:
        return "text"
    return "script"
...
def _reviewed_lines(text: str) -> set[int]:
    ...
    return {
        ...
        if _SECURITY_REVIEW_MARKER in line
    }
...
def validate_changed_source(root: Path, paths: list[Path]) -> list[str]:
    ...
    for path in paths:
        ...
        # Tests necessarily contain prohibited examples, and the guard contains
        # the policy strings themselves. They remain syntax-checked and are
        # exercised by the dedicated CI-tool tests; token enforcement targets
        # changed production and package source.
        if _is_guard_fixture(relative):
            continue

        reviewed = _reviewed_lines(text)
        if tree is not None:
            for node in ast.walk(tree):
                if (
                    ...
                    and node.lineno not in reviewed
                ):
                    errors.append(
                        f"{relative}:{node.lineno}: builtin eval is prohibited"
                    )
                if isinstance(node, ast.Call):
                    for keyword in node.keywords:
                        ...
                        if (
                            ...
                            errors.append(
                                f"{relative}:{line}: shell=True is prohibited"
                            )
                        if (
                            keyword.arg == "verify"
                            and isinstance(keyword.value, ast.Constant)
                            ...
                            errors.append(
                                f"{relative}:{line}: verify=False requires an isolated reviewed exception"
                            )
        for number, line in enumerate(text.splitlines(), start=1):
            if _SECURITY_REVIEW_MARKER in line:
                continue
            for token, message in BANNED_TEXT.items():
                if token in line and not is_comment_only_match(line, token):
                    errors.append(
                        f"{relative}:{number}: {message} ({token})"
                    )
    return errors
...
def validate_changed_packages_have_tests(root: Path, paths: list[Path]) -> list[str]:
    ...
    for path in paths:
        ...
        if package is None or not path.is_file() or _source_language(path) is None:
            continue
