...
manifest digests into workflow files and records the resolved source tags in a
...
def clean_suffix(suffix: str, source_ref: str) -> str:
    comment = f"# {source_ref}"
    if not suffix.strip():
        return f" {comment}"
...
def pin_workflow(path: Path, *, write: bool) -> tuple[dict[str, Any], bool]:
    ...
    for line in original.splitlines(keepends=True):
        ...
        if action_match:
            ...
            source_ref = ref
            if FULL_SHA_RE.fullmatch(ref):
                comment = action_match.group("suffix").strip().lstrip("#").strip()
                source_ref = comment or ref
                sha = ref
            else:
            ...
            replacement = (
                ...
                + "@"
                + sha
                + clean_suffix(action_match.group("suffix"), source_ref)
            )
            body = replacement
    ...
    for path in workflow_files(root):
        ...
        for number, line in enumerate(text.splitlines(), start=1):
            ...
            for image in IMAGE_RE.finditer(line):
                errors.append(
                    f"{path.relative_to(root)}:{number}: mutable image "
                    f"{image.group(0)}"
                )
    return errors
