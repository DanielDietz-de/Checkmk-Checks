...
IMAGE_TAGGED_DIGEST_RE = re.compile(
    r"(?P<repo>checkmk/check-mk-(?:raw|community)):"
    r"(?P<tag>[0-9][A-Za-z0-9._-]*)@"
...
def pin_workflow(path: Path, *, write: bool) -> tuple[dict[str, Any], bool]:
    ...
    for line in original.splitlines(keepends=True):
        ...
        for pinned in IMAGE_TAGGED_DIGEST_RE.finditer(body):
            repository = pinned.group("repo")
            tag = pinned.group("tag")
            digest = pinned.group("digest")
            pinned_spans.append(pinned.span())
...
def validate_locked_workflows(root: Path) -> list[str]:
    ...
    for path in workflow_files(root):
        ...
        for number, line in enumerate(text.splitlines(), start=1):
            ...
            pinned_spans = [
                match.span() for match in IMAGE_TAGGED_DIGEST_RE.finditer(line)
            ] + [match.span() for match in IMAGE_DIGEST_RE.finditer(line)]
            for image in IMAGE_TAG_RE.finditer(line):
                if not _inside_any_span(image.start(), pinned_spans):
                    errors.append(
